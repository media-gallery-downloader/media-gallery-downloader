<div
    x-data="{ 
        ...
        currentUploadId: @js($currentUploadId)
    }"
    @refresh-upload-queue.window="
        uploadQueue = [...$wire.uploadQueue];
        currentUploadId = $wire.currentUploadId;
    "
    ...
    <div class="flex justify-between items-center mb-3">
        ...
        <div class="flex gap-2" x-show="uploadQueue.length > 0">
            <button
                @click="$wire.clearUploadQueue()"
    ...
    <div x-show="uploadQueue.length === 0" class="text-sm text-gray-500 dark:text-gray-400 italic">
        Upload queue is empty.
    </div>

    <ul class="space-y-2" x-show="uploadQueue.length > 0">
                ...
                <div class="flex justify-between items-start">
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                            Type: <span x-text="item.mime_type || 'unknown'"></span> •
                            Added: <span x-text="new Date(item.added_at).toLocaleString()"></span>
                        </div>
                    </div>
                    <div class="ml-4 flex items-center gap-2">
                        <template x-if="currentUploadId === item.id">
                            <div class="flex flex-col items-end">
                                <span class="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded dark:bg-blue-900 dark:text-blue-300 animate-pulse flex items-center">
                                    ...
                                    Processing <span x-show="item.progress" x-text="'(' + Math.round(item.progress) + '%)'" class="ml-1"></span>
                                </span>
                        ...
                        <button
                            @click="$wire.cancelUpload(item.id)"
                            title="Cancel Upload"
